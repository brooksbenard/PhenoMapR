...
if (is.null(.meta_path)) {
  stop(
    "Could not obtain CRA001160 cell metadata. Place ",
    .meta_name, " under vignettes/ or set PHENOMAPR_CRA001160_META_URL.",
    call. = FALSE
  )
}
...
# Expression matrix from H5 when available; otherwise Seurat RDS (CI secret).
if (!is.null(.h5_path)) {
  expr_mat <- Seurat::Read10X_h5(.h5_path)
} else {
  message(
    "CRA001160 HDF5 unavailable (often Google Drive quota). ",
    "Falling back to Seurat RDS via local file or PHENOMAPR_CRA001160_RDS_URL."
  )
  ...
  if (is.null(.rds_path)) {
    stop(
      "Could not obtain CRA001160 expression (H5 or Seurat RDS). ",
      "Set PHENOMAPR_CRA001160_H5_URL or PHENOMAPR_CRA001160_RDS_URL ",
      "to a non-Drive mirror, or place the file under vignettes/.",
      call. = FALSE
    )
  }
