...
# Reject Drive "quota exceeded" JSON stubs so we can fall back to Seurat RDS.
.h5_name <- "PAAD_CRA001160_expression.h5"
.meta_name <- "PAAD_CRA001160_CellMetainfo_table.tsv"
.rds_name <- "PAAD_CRA001160_seurat.rds"
...
.find_packaged_cra_subset <- function() {
  candidates <- c(
    file.path("inst", "extdata", "vignette_subsets",
              "PAAD_CRA001160_vignette_subset.rds"),
    file.path("..", "inst", "extdata", "vignette_subsets",
              "PAAD_CRA001160_vignette_subset.rds"),
    system.file(
      "extdata", "vignette_subsets", "PAAD_CRA001160_vignette_subset.rds",
      package = "PhenoMapR"
    ),
    file.path("inst", "extdata", "shiny", "PAAD_CRA001160_demo_5000.rds"),
    system.file(
      "extdata", "shiny", "PAAD_CRA001160_demo_5000.rds",
      package = "PhenoMapR"
    )
  )
  candidates <- unique(candidates[nzchar(candidates)])
  for (p in candidates) {
    if (isTRUE(.is_valid_rds_blob(p))) return(p)
  }
  NULL
}
.normalize_cra_meta <- function(meta_df) {
  # Accept either TISCH column names or the packaged Shiny-demo schema.
  if ("Celltype (original)" %in% names(meta_df)) {
    ...
    maj <- as.character(meta_df$cell_type_major)
    meta_df <- data.frame(
      Cell = as.character(meta_df$.cell_id),
      UMAP_1 = as.numeric(meta_df$UMAP_1),
      UMAP_2 = as.numeric(meta_df$UMAP_2),
      celltype_malignancy = unname(
        ifelse(maj %in% names(map_mal), map_mal[maj], "Others")
      ),
      celltype_original = gsub(
        " cell$", "", as.character(meta_df$cell_type_original)
      ),
      Patient = as.character(meta_df$Patient),
      Source = as.character(meta_df$Source),
...
.load_cra_from_packaged_subset <- function(path) {
  message("Using packaged CRA001160 subset: ", path)
  obj <- readRDS(path)
  if (is.list(obj) && !is.null(obj$expression) && !is.null(obj$metadata)) {
    ...
    list(
      expression = expr,
      metadata = .normalize_cra_meta(as.data.frame(obj@meta.data))
    )
  } else {
    NULL
  }
...
# On CI / pkgdown Actions, prefer the committed subset and skip flaky Drive
# downloads of the full H5/Seurat objects (quota JSON stubs).
.prefer_packaged_subset <- nzchar(Sys.getenv("CI", unset = "")) ||
  nzchar(Sys.getenv("GITHUB_ACTIONS", unset = "")) ||
  identical(Sys.getenv("PHENOMAPR_VIGNETTE_USE_SUBSET", unset = ""), "true")
if (isTRUE(.prefer_packaged_subset)) {
  .packaged <- .find_packaged_cra_subset()
  if (!is.null(.packaged)) {
    loaded <- .load_cra_from_packaged_subset(.packaged)
    if (!is.null(loaded)) {
...
.h5_path <- if (is.null(expr_mat)) {
  .resolve_vignette_file(
    .h5_name,
...
# Prefer full H5 + metadata; else Seurat RDS; else packaged subset for CI.
if (is.null(expr_mat) && !is.null(.h5_path)) {
...
if (is.null(expr_mat)) {
  message(
    "CRA001160 HDF5 unavailable (often Google Drive quota). ",
    "Trying Seurat RDS via local file or PHENOMAPR_CRA001160_RDS_URL."
  )
  .rds_path <- .resolve_vignette_file(
    .rds_name,
    "14p_fYIFeuuRdXBF3J-5ZsXElq_mduSzb",
    "PHENOMAPR_CRA001160_RDS_URL",
    .is_valid_rds_blob
  )
  if (!is.null(.rds_path)) {
    .seu <- readRDS(.rds_path)
    if (is.list(.seu) && !is.null(.seu$expression)) {
      loaded <- .load_cra_from_packaged_subset(.rds_path)
      expr_mat <- loaded$expression
      meta <- loaded$metadata
    } else {
      expr_mat <- tryCatch(
        PhenoMapR:::.get_assay_data_compat(.seu, assay = "RNA", slot = "counts"),
        error = function(e) {
          PhenoMapR:::.get_assay_data_compat(.seu, assay = "RNA", slot = "data")
        }
      )
      rm(.seu)
      gc()
...
if (is.null(expr_mat) || is.null(meta)) {
  ...
  if (is.null(.packaged)) {
    stop(
      "Could not obtain CRA001160 expression/metadata. ",
      "Place H5+TSV or Seurat RDS under vignettes/, set ",
      "PHENOMAPR_CRA001160_*_URL secrets to a non-Drive mirror, ",
      "or keep inst/extdata/vignette_subsets/PAAD_CRA001160_vignette_subset.rds ",
      "in the checkout for CI/pkgdown.",
      call. = FALSE
    )
  }
  loaded <- .load_cra_from_packaged_subset(.packaged)
  if (is.null(loaded)) {
    stop("Packaged CRA001160 subset was unreadable: ", .packaged, call. = FALSE)
  }
  expr_mat <- loaded$expression
  meta <- loaded$metadata
}

# Keep metadata aligned to scored cells (important for subsampled CI builds).
...
rownames(meta) <- NULL
message(
  "CRA001160 load: ", nrow(expr_mat), " genes x ", ncol(expr_mat),
