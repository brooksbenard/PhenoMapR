...
```
Indeed, tumor samples are significantly enriched for more adverse scores across all cell types.
...
Here, we summarize the enrichment of prognostic PhenoMapR scores across samples and cell types to highlight the difference in signal between healthy and tumor samples as well as between different cell populations across the sample groups. The **top panel** shows a **pseudobulk** sample score: the mean cell-level PhenoMapR score within each sample, **z-scaled across samples** (so bars are comparable for relative high/low bulk-like signal per sample).
...
## Pseudobulk: mean cell-level PhenoMapR score per sample (all cells), z-scaled across samples
pb_mean <- tapply(as.numeric(meta[[score_precog_col]]), meta[[sample_col]], mean, na.rm = TRUE)
pb_mean <- as.numeric(pb_mean[as.character(sample_lev)])
pb_mean[is.na(pb_mean)] <- NA_real_
...
p_pbulk <- ggplot(pb_df, aes(x = as.numeric(.data$sample), y = .data$pb_score_scaled)) +
  ...
  labs(y = "Pseudobulk score\n(z across samples)", title = NULL)
