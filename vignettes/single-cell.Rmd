...
# Full CRA001160 cohort from the original TISCH2 H5 + metadata TSV.
# Do not use PAAD_CRA001160_full.rds / Seurat counts here: those matrices
# differ from the log-normalized H5 used in the long-standing vignette.
# All Tumor + Normal cells are retained (no subsetting / source filter).
...
if (is.null(.h5_path) || is.null(.meta_path)) {
  stop(
    "Could not obtain the FULL CRA001160 H5 expression matrix and metadata TSV. ",
    "Place PAAD_CRA001160_expression.h5 and ",
    "PAAD_CRA001160_CellMetainfo_table.tsv under vignettes/, or set ",
    "PHENOMAPR_CRA001160_H5_URL / PHENOMAPR_CRA001160_META_URL to a ",
...
# Expression matrix (TISCH2 log-normalized H5) and cell/sample metadata
...
# Keep every cell shared by H5 and metadata (full cohort: Tumor + Normal).
...
if (length(cells_keep) < ncol(expr_mat) || length(cells_keep) < nrow(meta)) {
  message(
    "Aligning H5 and metadata on shared cell IDs: keeping ",
    length(cells_keep), " / ", ncol(expr_mat), " H5 columns and ",
    length(cells_keep), " / ", nrow(meta), " metadata rows."
  )
}
...
rownames(meta) <- NULL
message(
  "CRA001160 load (H5, all cells): ", nrow(expr_mat), " genes x ",
  ncol(expr_mat), " cells (", length(unique(meta$Patient)), " patients; ",
  paste(names(table(meta$Source)), table(meta$Source), sep = "=", collapse = ", "),
  ")."
)
