...
```{r cytospace-spatialcooccur, fig.width=12, fig.height=10, out.width=NULL}
scoc_nhood <- NULL
...
if (!is.null(scoc_nhood) && !is.null(scoc_nhood$zscore)) {
  ...
  if (length(labs_ord) < 2L) {
    ...
    pal_pg <- c(Adverse = "#B2182B", Favorable = "#2166AC", Other = "#bdbdbd")
    ...
    if (requireNamespace("ComplexHeatmap", quietly = TRUE) && requireNamespace("circlize", quietly = TRUE)) {
      ...
      col_fun <- circlize::colorRamp2(c(-mabs, 0, mabs), c("#2166AC", "white", "#B2182B"))

      row_ha <- ComplexHeatmap::rowAnnotation(
        CellType = row_ct,
        Prognostic = row_pg,
        col = list(CellType = pal_ct, Prognostic = pal_pg),
      ...
      ht <- ComplexHeatmap::Heatmap(
        ...
        column_title = "Neighborhood enrichment (spatialCooccur::nhood_enrichment)\nZ-score vs permuted labels (rows = cell label, columns = neighbor label)",
    ...
    if (!drew_ch) {
      ij <- expand.grid(i = seq_len(nrow(mat)), j = seq_len(ncol(mat)), stringsAsFactors = FALSE)
      zlong_plot <- data.frame(
        from = rownames(mat)[ij$i],
        to = colnames(mat)[ij$j],
        z = mat[cbind(ij$i, ij$j)],
        stringsAsFactors = FALSE
      )
      message("Using ggplot tile heatmap (install ComplexHeatmap + circlize for annotated heatmap).")
      p_nh <- ggplot(zlong_plot, aes(x = .data$to, y = .data$from, fill = .data$z)) +
        ...
        labs(
          ...
          subtitle = "Z-scores for co-occurrence of CellType_prognostic_group labels (ggplot fallback)",
          x = "Neighbor label",
          y = "Cell label"
        ) +
