...
if (exists("has_spatial_coords") && has_spatial_coords && !is.null(group_col)) {
  spatial_df$prognostic_zorder <- ifelse(spatial_df$prognostic_group == "Other", 0L, 1L)
  p2 <- ggplot(spatial_df, aes(x = .data$row, y = -.data$col, color = .data$prognostic_group,
    size = points_per_location, zorder = .data$prognostic_zorder)) +
    geom_jitter(alpha = 0.8, width = spatial_jitter_w, height = spatial_jitter_h, shape = 16) +
...
if (!is.null(group_col)) {
  ...
  for (a in unique(c(assay_use, "RNA", "SCT"))) {
    ...
    if (!is.null(markers) && (nrow(markers$adverse_markers) > 0 || nrow(markers$favorable_markers) > 0)) break
  }
  if (!is.null(markers)) {
...
if (exists("markers") && !is.null(markers)) {
  ...
  for (a in unique(c(assay_use, "RNA", "SCT"))) {
    if (a %in% names(seurat@assays)) {
      ...
      if (!is.null(expr) && nrow(expr) > 0) break
    }
  }
  if (!is.null(expr)) {
  ...
  if (requireNamespace("pheatmap", quietly = TRUE)) {
    ...
    heatmap_colors <- if (requireNamespace("viridis", quietly = TRUE)) viridis::viridis(100) else colorRampPalette(c("#2166AC", "#F7F7F7", "#B2182B"))(100)
    pheatmap::pheatmap(
      mat_scaled,
      scale = "none",
      cluster_cols = FALSE,
    ...
    message("Could not retrieve expression data for heatmap.")
  }
}
```
...
```{r markers-by-celltype-spatial, cache = FALSE}
markers_by_ct <- NULL
ct_col_markers <- NULL
if (!is.null(group_col)) {
  ...
  if (exists("spatial_df_celltype_col") && !is.null(spatial_df_celltype_col) && spatial_df_celltype_col %in% names(seurat@meta.data)) {
    ...
    for (c in c("CellType", "Celltype..major.lineage.", "cell_type", "celltype", "annotation")) {
      if (c %in% names(seurat@meta.data) && length(unique(seurat@meta.data[[c]])) >= 2) {
  ...
  if (!is.null(celltype_for_markers)) {
    ...
    meta$combined_group <- paste0(tolower(gsub(" ", "_", meta$pg)), "_", gsub(" ", "_", meta$ct))
    idx_keep <- meta$pg %in% c("Most Adverse", "Most Favorable")
    combined_tab <- table(meta$combined_group[idx_keep], useNA = "no")
    combined_lev <- names(combined_tab)[combined_tab >= 10]
    if (length(combined_lev) >= 1L) {
      cells_keep <- rownames(meta)[idx_keep & meta$combined_group %in% combined_lev]
      seurat_sub <- seurat[, cells_keep]
      seurat_sub$PhenoMapR_combined_group <- meta[cells_keep, "combined_group"]
      Seurat::Idents(seurat_sub) <- seurat_sub$PhenoMapR_combined_group
      assay_fa <- if (assay_use %in% names(seurat_sub@assays)) assay_use else "RNA"
      ...
      if (!is.null(markers_by_ct) && nrow(markers_by_ct) > 0) {
        ...
        message("Found markers for ", length(unique(markers_by_ct$cluster)), " adverse/favorable cell type groups.")
        print(head(markers_by_ct[, c("gene", "cluster", "avg_log2FC", "p_val_adj")], 10))
      }
    } else {
      message("Insufficient cells in adverse/favorable cell type combinations (need >= 10 per group); skipping.")
    }
  } else {
    message("No cell type column found; skipping cell-type-specific markers.")
  }
}
```
...
```{r heatmap-markers-by-celltype-spatial}
if (exists("markers_by_ct") && !is.null(markers_by_ct) && nrow(markers_by_ct) > 0 && exists("ct_col_markers") && !is.null(ct_col_markers)) {
  ...
  if (length(top_genes_ct) > 0) {
    ...
    meta$combined_group <- paste0(tolower(gsub(" ", "_", meta$pg)), "_", gsub(" ", "_", meta$ct))
    meta$combined_group[meta$pg == "Other"] <- "Other"
