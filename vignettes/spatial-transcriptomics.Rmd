...
if (!is.null(scoc_nhood) && !is.null(scoc_nhood$zscore)) {
  ...
  # Plot only Adverse/Favorable labels (keep *_Other in results, but hide in plot)
  keep_lab <- unique(c(
    zlong$from[grepl("_(Adverse|Favorable)$", zlong$from)],
    zlong$to[grepl("_(Adverse|Favorable)$", zlong$to)]
  ))
  zlong_plot <- zlong[zlong$from %in% keep_lab & zlong$to %in% keep_lab, , drop = FALSE]
