...
if (!requireNamespace("spatialCooccur", quietly = TRUE)) {
  ...
  # compare CellType_Most Adverse neighborhoods for presence of CellType_Most Favorable (and vice versa).
  df_local_list <- list()
  cell_types <- sort(unique(sub("_Most (Adverse|Favorable|Other)$", "", unique(scoc_df$ct_pg))))
  for (ct in cell_types) {
    xlab <- paste0(ct, "_Most Adverse")
    ylab <- paste0(ct, "_Most Favorable")
...
if (!is.null(scoc_nhood) && !is.null(scoc_nhood$zscore)) {
  ...
  p_nh <- ggplot(zlong, aes(x = .data$to, y = .data$from, fill = .data$z)) +
    ...
    labs(
      ...
      subtitle = "Z-scores for co-occurrence of CellType_prognostic_group labels vs permuted labels",
      x = "Neighbor label",
      y = "Cell label"
    ) +
