...
if (!requireNamespace("spatialCooccur", quietly = TRUE)) {
  message("Install spatialCooccur for co-localization: remotes::install_github(\"juninamo/spatialCooccur\")")
           ...
           !all(c("Cell", "row", "col", "prognostic_group") %in% names(cell_locations))) {
  message("Co-localization skipped: expected `cell_locations` with columns Cell, row, col, prognostic_group.")
} else {
  dfb <- cell_locations[
    stats::complete.cases(cell_locations[, c("Cell", "row", "col", "prognostic_group")]),
    ,
    drop = FALSE
  ]
  ...
  dfb <- dfb[dfb$prognostic_group %in% c("Most Adverse", "Most Favorable", "Other"), , drop = FALSE]

  scoc_df <- data.frame(
    x = dfb$row,
    y = dfb$col,
    prognostic_group = dfb$prognostic_group,
