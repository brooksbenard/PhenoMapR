...
Below we run all three contrasts and draw **`plot_phenotype_markers()`** heatmaps with **five genes per block** and **five `anno_mark` labels per block** (`top_n_markers = 5`, `n_mark_labels = 5`). Gene labels are colored by cell type (`color_mark_labels_by_celltype = TRUE`) on the cell-type-specific heatmaps.
...
if (!is.null(expr_pm) && !is.null(markers_ct_opp)) {
  PhenoMapR::plot_phenotype_markers(
...
if (!is.null(expr_pm) && !is.null(markers_ct_within)) {
  PhenoMapR::plot_phenotype_markers(
