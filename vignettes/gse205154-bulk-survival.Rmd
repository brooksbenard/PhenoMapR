...
# If the RDS holds a list (e.g. from getGEO), take the first ExpressionSet
...
# Optionally set rownames to HUGO symbols if feature data are available and aligned
...
if (!is.null(fd) && nrow(fd) == nrow(expr)) {
  ...
  if (!is.na(sym_col)) {
    ...
    if (sum(!is.na(symbols)) > 0) {
      # Keep only genes with non-NA symbols and set rownames
      keep <- !is.na(symbols)
...
expr <- as.matrix(expr)
expr[expr < 0] <- 0
expr_tpm <- apply(expr, 2, function(x) {
