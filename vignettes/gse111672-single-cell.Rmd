...
rds_path <- file.path(vignette_dir, "PAAD_GSE111672_seurat.rds")
...
if (!has_data) {
  message("Could not download PAAD_GSE111672_seurat.rds from Google Drive.")
} else {
  t0 <- Sys.time()
  ...
  rds_path2 <- file.path(vignette_dir, "PAAD_CRA001160_seurat.rds")
...
```{r load-cra001160}
if (is.null(seurat2)) {
  vignette_dir2 <- if (dir.exists("vignettes")) "vignettes" else if (dir.exists("Vignettes")) "Vignettes" else "."
  rds_path2 <- file.path(vignette_dir2, "PAAD_CRA001160_seurat.rds")
  googledrive::drive_deauth()
  googledrive::drive_download(googledrive::as_id("14p_fYIFeuuRdXBF3J-5ZsXElq_mduSzb"), rds_path2, overwrite = TRUE)
  if (file.exists(rds_path2)) {
    t0 <- Sys.time()
